...
pub async fn read_block(
    ...
    if length > packet_size {
        ...
        for i in 0..p {
            ...
            commands.push(read_block(ctx.clone(), reader.clone().into_string().unwrap(), block, cancel_flag, Some(size), Some(block_size), Some(packet_size)));
        }
                            ...
                            if reader_state.event_state().contains(State::PRESENT) {
...

                                {
                                    ...
                                    return if atr.starts_with(&[0x3B, 0x8F, 0x80, 0x01, 0x80, 0x4F]) {
                                        ...
                                        authenticate_14443_3(&tx, block_number as u8, KeyType::A).map_err(|e| e)?;
    ...
    async fn test_read_block() {
        ...
        match result.await {
            Ok(data) => {
                ...
                let data_str = data.iter().map(|b| format!("{:02X}", b)).collect::<Vec<String>>().join(" ");
                // Convert the hex to readable text
                let data_text = data_str.split_whitespace().map(|s| {
