...
/// Writes data to a specified block on a card.
///
/// # Arguments
///
...
/// # Returns
///
/// * `Ok(true)` if the write operation is successful.
...
/// let result = write_block(ctx, reader, 4, data, Some(16), &cancel_flag).await;
...
pub(crate) async fn write_block(
    ...
    if data.len() < block_size as usize || data.len() % block_size as usize != 0 {
        return Err(ReaderError::CardError(
            "Data length must be a multiple of block size.".to_string(),
...

                                {
                                    ...
                                    return if atr.starts_with(&[0x3B, 0x8F, 0x80, 0x01, 0x80, 0x4F])
                                    {
                                        ...
                                        if status_code != 0x9000 {
                                            return Err(ReaderError::CardError(
                                                format!("Write failed with code: {}", status_code),
                                                Error::InvalidParameter,
                                            ));
                                        }
...
mod tests {
    ...
    async fn test_write_multiple() {
        ...
        block_four[..block_four_copy_len]
            .copy_from_slice(&block_four_data.as_bytes()[..block_four_copy_len]);
        ...
        block_five[..block_five_copy_len]
            .copy_from_slice(&block_five_data.as_bytes()[..block_five_copy_len]);
        ...
        block_six[..block_six_copy_len]
            .copy_from_slice(&block_six_data.as_bytes()[..block_six_copy_len]);
        ...
        block_eight[..block_eight_copy_len]
            .copy_from_slice(&block_eight_data.as_bytes()[..block_eight_copy_len]);
